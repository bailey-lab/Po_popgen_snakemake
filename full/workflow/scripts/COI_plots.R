...
combined_COI$species_ordered <- factor(combined_COI$species, levels = c("P. ovale curtisi (n=13)", "P. ovale wallikeri (n=16)", "P. falciparum (n=842)"))

COI <- combined_COI %>% 
  ggplot() +
  geom_violin(aes(x = species_ordered, y = Median.COI, color = species)) +
  scale_color_manual(values = c("P. ovale curtisi (n=13)"="green4","P. ovale wallikeri (n=16)"="skyblue3","P. falciparum (n=842)"="orange3")) +
...
COI
ggsave("COI/COI_ov1.png", plot = last_plot(), device = "png", dpi = 600, width = 9, height = 5)
