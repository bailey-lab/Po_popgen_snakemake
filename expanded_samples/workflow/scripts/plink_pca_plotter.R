...
poc <- read.table("Revised - P. o. pop genomics data tables - Poc PCA r2-0.3.tsv", header = TRUE, sep = "\t")
poceigenval <- read.table("Revised - P. o. pop genomics data tables - Poc Eigenvalues r2-0.3.tsv", header = FALSE, sep = "\t")
...
for (i in 1:length(poceigenval$eigenvalues)){
  colnames(poc)[i+4] <- paste(colnames(poc)[i+4]," (",poceigenval$percent[i],"%)", sep = "")
}

#PCA plots for geographical clustering from plink
library(ggplot2)

old_pca_colors <- c("Tanzania (East)"="#E31A1C", "Tanzania (West)" = "#FB9A99", "Kenya" = "#FF7F00", "South Sudan" = "#FDBF6F", "Ethiopia"="#FFFF99",  "DRC (East)"= "#B2DF8A", "DRC (West)" = "#33A02C", "Congo" = "#A6CEE3", "Gabon"="#1F78B4", "Cameroon" = "#CAB2D6", "Nigeria" = "#6A3D9A", "Ghana" = "#B15928", "Ivory Coast"="#A6761D","Sierra Leone"="#666666", "Senegal" = "black" )

pca_region_colors <- c("East Africa" = "#1B9E77", "West Africa" = "#7570B3", "Central Africa" = "#D95F02")

pca_country_shapes <- c("Tanzania (East)"=15, "Tanzania (West)" = 16, "Kenya" = 17, "South Sudan" = 18, "Ethiopia"=8,  "DRC (East)"= 15, "DRC (West)" = 16, "Congo" = 17, "Gabon"=18, "Cameroon" = 8, "Nigeria" = 15, "Ghana" = 16, "Ivory Coast"=17,"Sierra Leone"=18, "Senegal" = 8)


poc$format <- "format"
ggplot(poc, aes(`PC1 (7.8%)`,`PC2 (7.2%)`, color=as.factor(Region), shape = as.factor(Country))) + geom_point(size = 5) + scale_color_manual(values = pca_region_colors) + scale_shape_manual(values = pca_country_shapes)
ggplot(poc, aes(`PC1 (7.8%)`,`PC2 (7.2%)`, color=as.factor(Region), label = IID)) + geom_text() + geom_point(size = 5) + scale_color_manual(values = pca_region_colors)
#poc$`-PC1 (12.3%)` <- -1*poc$`PC1 (12.3%)`
#poc$`-PC2 (12%)` <- -1*poc$`PC2 (12%)`
ggplot(poc, aes(`PC1 (7.8%)`,`PC2 (7.2%)`, color=as.factor(Region),shape = as.factor(Country))) + geom_point(size = 5) + labs(color = "Region of origin") + ggtitle("PCA of P. ovale curtisi samples") + theme(plot.title = element_text(hjust =0.5, size =30),axis.title = element_text(size = 30),axis.text= element_text(size = 14), title = element_text(size = 20), legend.text = element_text(size = 15)) + theme_bw(base_size = 15)+ scale_color_manual(values = pca_region_colors)+ scale_shape_manual(values = pca_country_shapes)
ggplot(poc, aes(`PC1 (7.8%)`,`PC2 (7.2%)`, color=as.factor(Region),shape = as.factor(Country))) + geom_point(size = 5) + labs(color = "Region of origin") + ggtitle("P. ovale curtisi") + theme(plot.title = element_text(hjust =0.5, size =30),axis.title = element_text(size = 30),axis.text= element_text(size = 14), title = element_text(size = 20), legend.text = element_text(size = 15)) + theme_bw(base_size = 15)+ scale_color_manual(values = pca_region_colors) + scale_shape_manual(values = pca_country_shapes, guide = NULL)
ggsave("PCA/poc_pc1-pc2.png", plot = last_plot(), device = "png", dpi = 600, width = 6, height = 5)
ggplot(poc, aes(`PC1 (7.8%)`,`PC3 (7%)`, color=as.factor(Region), shape = as.factor(Country))) + geom_point(size = 5) + labs(color = "Country of origin") + ggtitle("PCA of P. ovale curtisi samples") + theme(plot.title = element_text(hjust =0.5, size =30),axis.title = element_text(size = 30),axis.text= element_text(size = 14), title = element_text(size = 20), legend.text = element_text(size = 15)) + theme_bw(base_size = 15)+ scale_color_manual(values = pca_region_colors) + scale_shape_manual(values = pca_country_shapes)
#PC3 separates Tanzanian and east DRC samples from others
ggplot(poc, aes(`PC1 (7.8%)`,`PC4 (5.9%)`, color=as.factor(Region), shape = as.factor(Country))) + geom_point(size = 5) + labs(color = "Country of origin") + ggtitle("PCA of P. ovale curtisi samples") + theme(plot.title = element_text(hjust =0.5, size =30),axis.title = element_text(size = 30),axis.text= element_text(size = 14), title = element_text(size = 20), legend.text = element_text(size = 15)) + theme_bw(base_size = 15) + scale_color_manual(values = pca_region_colors) + scale_shape_manual(values = pca_country_shapes)
#PC4 separates cameroon and south sudan from Nigeria, Sierra Leone, DRC, and Tanzania

###P. o. wallikeri
#Load in Pow datatables (including country of origin and eigenvalues)
pow <- read.table("Revised - P. o. pop genomics data tables - Pow PCA r2-0.3.tsv", header = TRUE, sep = "\t")
poweigenval <- read.table("Revised - P. o. pop genomics data tables - Pow Eigenvalues r2-0.3.tsv", header = FALSE, sep = "\t")
...
for (i in 1:length(poweigenval$eigenvalues)){
  colnames(pow)[i+4] <- paste(colnames(pow)[i+4]," (",poweigenval$percent[i],"%)", sep = "")
}
...
pow$format <- "format"
ggplot(pow, aes(`PC1 (9.8%)`,`PC2 (7.7%)`, color=as.factor(Region), shape = as.factor(Country))) + geom_point(size = 5) + scale_color_manual(values = pca_region_colors)+ labs(color = "Country of origin") + ggtitle("PCA of P. ovale wallikeri samples") +theme_bw(base_size = 15) + scale_shape_manual(values = pca_country_shapes)
#pow$`-PC2 (9.8%)` <- -1*pow$`PC2 (9.8%)`
ggplot(pow, aes(`PC1 (9.8%)`,`PC2 (7.7%)`, color=as.factor(Region), shape = as.factor(Country),label=IID)) + geom_point(size = 5) + geom_text() + scale_color_manual(values = pca_region_colors) + ggtitle("PCA of P. ovale wallikeri samples") +theme_bw(base_size = 15) + scale_shape_manual(values = pca_country_shapes, guide = NULL)
ggplot(pow, aes(`PC1 (9.8%)`,`PC2 (7.7%)`, color=as.factor(Region), shape = as.factor(Country))) + geom_point(size = 5) + scale_color_manual(values = pca_region_colors)+ labs(color = "Region of origin") + ggtitle("PCA of P. ovale wallikeri samples") +theme_bw(base_size = 15)+ scale_y_continuous(breaks = seq(-0.5, 0.75, by = 0.25)) + scale_shape_manual(values = pca_country_shapes, guide = NULL)
ggsave("PCA/pow_pc1-pc2.png", plot = last_plot(), device = "png", dpi = 600, width = 6, height = 4.75)
#PC3 splits the ivory coast sample from Gabon
ggplot(pow, aes(`PC1 (9.8%)`,`PC3 (6.1%)`, color=as.factor(Region), shape = as.factor(Country))) + geom_point(size = 5) + scale_color_manual(values = pca_region_colors)+ labs(color = "Country of origin") + ggtitle("PCA of P. ovale wallikeri samples") +theme_bw(base_size = 15) + scale_shape_manual(values = pca_country_shapes)
ggplot(pow, aes(`PC1 (9.8%)`,`PC3 (6.1%)`, label=IID)) + geom_point(size = 5) + geom_text() + scale_color_manual(values = pca_colors) + ggtitle("PCA of P. ovale wallikeri samples") +theme_bw(base_size = 15)
#PC4 splits Kinshasa and Congo samples from others
ggplot(pow, aes(`PC1 (9.8%)`,`PC4 (5.7%)`, color=as.factor(Region), shape = as.factor(Country))) + geom_point(size = 5)+ scale_color_manual(values = pca_region_colors)+ labs(color = "Country of origin") + ggtitle("PCA of P. ovale wallikeri samples") +theme_bw(base_size = 15) + scale_shape_manual(values = pca_country_shapes)
ggplot(pow, aes(`PC1 (9.8%)`,`PC4 (5.7%)`, label=IID)) + geom_point(size = 5) + geom_text() + scale_color_manual(values = pca_region_colors) + ggtitle("PCA of P. ovale wallikeri samples") +theme_bw(base_size = 15)

###arrange ovale plots next to each other
#install.packages("ggpubr")
library("ggpubr")

poc_pc1pc2 <- ggplot(poc, aes(`PC1 (7.8%)`,`PC2 (7.2%)`, color=as.factor(Region), shape = as.factor(Country))) + 
  geom_point(size = 5) + labs(color = "Region of origin") + theme_bw(base_size = 15)+
  ...
  scale_color_manual(values = pca_region_colors) + 
  scale_shape_manual(values = pca_country_shapes, guide = NULL) +
  scale_y_continuous(breaks = seq(-0.5, 0.75, by = 0.25)) + 
  scale_x_continuous(breaks = seq(-0.5, 0.5, by = 0.25))
poc_pc1pc2
ggsave("PCA/poc_pc1-pc2.png", plot = last_plot(), device = "png", dpi = 600, width = 6, height = 5)
pow_pc1pc2 <- ggplot(pow, aes(`PC1 (9.8%)`,`PC2 (7.7%)`, color=as.factor(Region), shape = as.factor(Country))) + 
  geom_point(size = 5) + scale_color_manual(values = pca_region_colors)+ 
  labs(color = "Region of origin") + theme_bw(base_size = 15)+ 
  theme(plot.title = element_text(hjust =0.5, size =30),axis.title = element_text(size = 24),axis.text= element_text(size = 20), title = element_text(size = 20), legend.text = element_text(size = 20)) + 
  scale_y_continuous(breaks = seq(-0.5, 0.75, by = 0.25)) + 
  scale_x_continuous(breaks = seq(-0.5, 0.5, by = 0.25)) + 
  scale_shape_manual(values = pca_country_shapes, guide = NULL)
...
ggarrange(poc_pc1pc2,pow_pc1pc2, ncol=2, nrow=1, common.legend = TRUE, legend="right", widths = c(5.1,4.75))
ggsave("PCA/poc-pow_pc1-pc2.png", plot = last_plot(), device = "png", dpi = 600, width = 11.5, height = 5)


poc_pc34_plot <- ggplot(poc, aes(`PC3 (7%)`,`PC4 (5.9%)`, color=as.factor(Region), shape = as.factor(Country))) + 
            geom_point(size = 5) + 
            theme_bw(base_size = 15) +
            labs(color = "Region of origin") + 
            theme(plot.title = element_text(hjust =0.5, size =30),axis.title = element_text(size = 24),axis.text= element_text(size = 20), title = element_text(size = 20), legend.text = element_text(size = 20)) + 
            scale_color_manual(values = pca_region_colors, drop = FALSE) +
            scale_shape_manual(values = pca_country_shapes, guide = NULL) +
...
pow_pc34_plot <- ggplot(pow, aes(`PC3 (6.1%)`,`PC4 (5.7%)`, color=as.factor(Region), shape = as.factor(Country))) + 
            geom_point(size = 5) + 
            theme_bw(base_size = 15) + 
            theme(plot.title = element_text(hjust =0.5, size =30),axis.title = element_text(size = 24),axis.text= element_text(size = 20), title = element_text(size = 20), legend.text = element_text(size = 20)) + 
            scale_color_manual(values = pca_region_colors, drop = FALSE) + 
