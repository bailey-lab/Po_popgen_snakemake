...
#Comment in/out specific file endpoints to adjust the portion of the pipeline that will be run
	input:
		...
		#sort_bothlanes = expand(config["output"]+"alignments_sorted/{samplename}_{lane}_{species}-pf3d7.bam", samplename = samples_to_merge, lane = ["L001","L002"], species = ["curtisigh01","wallikericr01"]),
		#sort_onelane = expand(config["output"]+"alignments_sorted/{samplename}_L001_{species}-pf3d7.bam", samplename = samples_no_merge, species = ["curtisigh01","wallikericr01"]),
		###merge across sequencing lanes for samples run on multiple lanes
		#merged = expand(config["output"]+"merged_alignments/{samplename}_{species}-pf3d7_sorted.bam", samplename = samples_to_merge, species = ["curtisigh01","wallikericr01"]),
		#unmerged = expand(config["output"]+"merged_alignments/{samplename}_{species}-pf3d7_sorted.bam", samplename = samples_no_merge, species = ["curtisigh01","wallikericr01"]),
